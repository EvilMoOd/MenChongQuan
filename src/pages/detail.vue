<script lang="ts" setup>
  ...
  import { useDetailStore } from '@/store/modules/detailStore';
  ...
  function getPostDetail() {
    //获取路由地址
    detail.getPostDetail(route.params.pid);
  }
  async function submitComment() {
    ...
    if (!localStorage.getItem('TOKEN')) {
      ...
      try {
        await detail.publishComment(rcontent, pid);
        location.reload();
        alert('发布成功');
      } catch (error: any) {
        alert(error.message);
...
<template>
  <div>
    <header class="full_page">
      <h1 class="title">{{ detail.pname }}</h1>
    </header>
    <main class="layout">
      <article class="post" v-for="(posts, index) in detail.floor" :key="posts.rid">
        <div class="user">
        ...
        <section>
          ...
          <div class="user-msg">
            ...
            <span class="time">{{ formDate(posts.rtime) }}</span>
          </div>
